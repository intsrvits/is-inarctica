...
def update_task():
    ...
    for cloud_task in all_cloud_tasks:
        ...
        if not box_task_id or box_task_id != 3209:
            continue
    ...
    update_batch_result = box_token.batch_api_call(update_methods, timeout=25)
    debug_point("Обновление пользовательских полей в задачах\n"
                f"Всего задач: {len(all_cloud_tasks)}\n"
                f"Обновлено: {len(update_batch_result.successes)}\n"
                f"C ошибкой: {len(update_batch_result.errors)}\n")
