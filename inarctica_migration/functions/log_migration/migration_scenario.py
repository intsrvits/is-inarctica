...
def project_log_migration():
    ...
    # Перенос для всех синхронизированных групп
    #for cloud_group_id in migrated_groups:
    for cloud_group_id in [107]:
        already_migrated = 0
        ...
        try:
            ...
            debug_point(error_log_message(exc, total_found=len(sorted_cloud_blogposts), migrated=len(bulk_data) + already_migrated, dest=cloud_group_id))
            ...
            debug_point(success_log_message(total_found=len(sorted_cloud_blogposts), migrated=len(bulk_data) + already_migrated, dest=cloud_group_id))
